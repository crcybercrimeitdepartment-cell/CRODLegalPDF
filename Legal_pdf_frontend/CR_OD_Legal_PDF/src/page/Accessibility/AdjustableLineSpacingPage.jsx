...
var presets = [
  ...
  { mult: 1.4, label: 'Normal (1.4x)' },
  { mult: 1.8, label: 'Relaxed (1.8x)' },
  { mult: 2.2, label: 'Double (2.2x)' },
  { mult: 2.6, label: 'Loose (2.6x)' },
];

var steps = [
  'Open PDF document',
  'Select Adjustable Line Spacing tool',
  'Choose line spacing multiplier level',
  'System updates document layout',
  'Review updated reading experience',
  'Apply settings & save updated PDF',
];

function getPresetName(mult) {
  if (Math.abs(mult - 1.1) < 0.05) return 'Tight';
  if (Math.abs(mult - 1.4) < 0.05) return 'Normal';
  if (Math.abs(mult - 1.8) < 0.05) return 'Relaxed';
  if (Math.abs(mult - 2.2) < 0.05) return 'Double';
  if (Math.abs(mult - 2.6) < 0.05) return 'Extra Loose';
  return 'Custom';
}

export default function AdjustableLineSpacingPage({ onBack }) {
  var [documentId, setDocumentId] = useState(null);
  var [fileStatus, setFileStatus] = useState('');
  var [lineSpacing, setLineSpacing] = useState(1.8);
  ...
  var fetchPreview = useCallback(async function () {
    ...
    try {
      var payload = {
        line_spacing_mult: lineSpacing,
        preset_name: getPresetName(lineSpacing),
        paragraph_gap_mult: paragraphGap,
        text_color_hex: '#1e293b',
      };
      var res = await fetch(API_BASE + '/line-spacing/' + documentId + '/extract', {
        method: 'POST',
  ...
  var handleLineSpacingChange = useCallback(function (newVal) {
    newVal = Math.max(1.0, Math.min(3.0, newVal));
    setLineSpacing(newVal);
    setAppliedMult(newVal);
  }, []);

  var applyLineSpacing = useCallback(async function () {
    if (!documentId) return;
    setApplying(true);
    try {
      var payload = {
      ...
      var res = await fetch(API_BASE + '/line-spacing/' + documentId + '/export', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      });
      ...
      alert('Line spacing error: ' + err.message);
    } finally {
      setApplying(false);
    }
  }, [documentId, lineSpacing, paragraphGap]);
    ...
    <div className="flex-1 flex flex-col w-full h-[calc(100vh-64px)] relative pt-11 sm:pt-4 bg-[#F5F3EC] overflow-hidden px-4 sm:px-8 lg:px-12 pb-4 sm:pb-8 font-sans">
      ...
      <div className="absolute top-1.5 left-4 sm:top-5 sm:left-8 md:left-12 z-50 flex items-center w-[calc(100%-2rem)] sm:w-[calc(100%-4rem)] md:w-[calc(100%-6rem)] min-h-[40px]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center pointer-events-none w-full px-20">
          <h1 className="text-xl sm:text-2xl font-extrabold text-[#1e2a52] tracking-tight drop-shadow-sm leading-tight">Adjustable Line Spacing Studio</h1>
                  </div>
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row w-full max-w-[1920px] mx-auto mt-12 sm:mt-16 lg:mt-20 border border-slate-200 bg-white overflow-hidden rounded-xl shadow-[0_15px_50px_rgba(0,0,0,0.05)] relative z-10">
        <div className="w-full lg:w-[340px] border-r border-slate-200 bg-slate-50 flex flex-col shrink-0 overflow-y-auto p-5 z-20">
          ...
          <div className="flex flex-col gap-3">
            <div className="text-xs font-semibold uppercase tracking-wider text-slate-500 flex items-center gap-1.5">
              <span className="text-base">{'\u2699'}</span> 2. Line Spacing Controls
            </div>

            <div className="flex flex-col gap-2">
              <label className="text-[11px] font-bold text-slate-600">Line Spacing Presets</label>
              <div className="flex flex-wrap gap-2">
                {presets.map(function (p) {
                  return (
                    <button key={p.mult} onClick={function () { handleLineSpacingChange(p.mult); }}
                      className={'px-3 py-2 rounded-full text-[13px] font-medium cursor-pointer border transition-all ' +
                        (Math.abs(lineSpacing - p.mult) < 0.05
            ...
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={function () { handleLineSpacingChange(lineSpacing - 0.1); }} disabled={lineSpacing <= 1.0}
                    className="w-9 h-9 flex items-center justify-center rounded-l-lg border border-r-0 border-slate-300 bg-white text-lg font-semibold cursor-pointer hover:bg-teal-50 hover:border-teal-400 hover:text-teal-600 disabled:opacity-40 disabled:cursor-not-allowed">-</button>
                  <span className="text-[15px] font-extrabold min-w-[45px] text-center" style={{ color: '#0d9488' }}>{lineSpacing.toFixed(1)}x</span>
                  <button onClick={function () { handleLineSpacingChange(lineSpacing + 0.1); }} disabled={lineSpacing >= 3.0}
                    className="w-9 h-9 flex items-center justify-center rounded-r-lg border border-l-0 border-slate-300 bg-white text-lg font-semibold cursor-pointer hover:bg-teal-50 hover:border-teal-400 hover:text-teal-600 disabled:opacity-40 disabled:cursor-not-allowed">+</button>
                </div>
              </div>
              <input type="range" min={1.0} max={3.0} step={0.1} value={lineSpacing} onChange={function (e) { handleLineSpacingChange(parseFloat(e.target.value)); }} className="w-full accent-teal-600" />
            </div>

            <div className="flex flex-col gap-2 pt-2.5 border-t border-slate-100">
              <div className="flex justify-between items-center">
                <span className="text-[12px] font-bold text-slate-600">Paragraph Gap</span>
                <span className="text-[13px] font-bold" style={{ color: '#0d9488' }}>{paragraphGap.toFixed(1)}x</span>
              </div>
              <input type="range" min={1.0} max={2.5} step={0.1} value={paragraphGap} onChange={function (e) { setParagraphGap(parseFloat(e.target.value)); }} className="w-full accent-teal-600" />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex justify-between text-[12px] font-semibold text-slate-600">
              <span>Formatted Elements: <strong style={{ color: '#0d9488' }}>{totalLines}</strong></span>
              <span>Applied Multiplier: <strong style={{ color: '#0d9488' }}>{appliedMult.toFixed(1)}x</strong></span>
            </div>
            <button onClick={applyLineSpacing} disabled={!documentId || applying}
              ...
              style={{ backgroundColor: applying ? '#94a3b8' : applied ? '#10b981' : '#0d9488' }}>
              <AlignCenter className="w-4 h-4" /> {applying ? 'Applying Line Spacing...' : applied ? '\u2713 Line Spacing Applied!' : 'Apply Custom Line Spacing'}
            </button>
            {downloadUrl && (
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-white">
          <div className="bg-slate-100 border-b border-slate-200 px-4 py-3 flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <AlignCenter className="w-4 h-4" /> Line Spacing Reader Canvas
            </div>
