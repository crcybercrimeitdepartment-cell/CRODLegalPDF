...
export default function PDFtoGIFPage({ onBack }) {
  const toolName = "PDF to GIF";
  const toolDesc = "Convert your PDF with PDF to GIF.";
  const apiSlug = "pdf-to-gif";
  const outputExt = ".zip";
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const dlUrl = processData.download_url || processData.zip_url || processData.animated_url || processData.image_urls?.[0] || processData.file_urls?.[0];
