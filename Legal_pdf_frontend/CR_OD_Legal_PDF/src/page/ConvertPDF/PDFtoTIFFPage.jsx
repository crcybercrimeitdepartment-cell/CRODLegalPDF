...
export default function PDFtoTIFFPage({ onBack }) {
  const toolName = "PDF to TIFF";
  const toolDesc = "Convert your PDF with PDF to TIFF.";
  const apiSlug = "pdf-to-tiff";
  const outputExt = ".zip";
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const dlUrl = processData.download_url || processData.zip_url || processData.multipage_url || processData.image_urls?.[0] || processData.file_urls?.[0];
