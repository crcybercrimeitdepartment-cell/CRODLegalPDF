...
import {
  UploadCloud, X, Sun, Download, Package, ArrowLeft, Image as ImageIcon
} from 'lucide-react';

const PRESETS = [
  { label: 'Dark', value: -50 },
  { label: 'Normal', value: 0 },
  { label: 'Bright', value: 50 },
  { label: 'Max', value: 100 }
];

const AdjustBrightness = ({ tool, onBack }) => {
  ...
  useEffect(() => {
    if (activeFile && activeFile.brightness !== brightness) {
      setUploadedFiles(prev => {
        const up = [...prev];
        if (up[activeIndex]) {
          up[activeIndex].brightness = brightness;
  ...
  }, [brightness, activeFile, activeIndex]);

  // Sync state when active index changes
  useEffect(() => {
    if (activeFile) {
      setBrightness(activeFile.brightness);
    }
  }, [activeIndex]);

  // Helper for Displaying brightness values (+/-)
  const formatBrightness = (val) => {
    if (val > 0) return `+${val}%`;
    if (val < 0) return `${val}%`;
    return '0%';
  };
  ...
  const applyBrightness = async () => {
    ...
    try {
      ...
      // Frontend Simulation using Canvas Filters: brightness(100% is normal)
      const cssBrightness = 100 + brightness;
      ctx.filter = `brightness(${Math.max(0, cssBrightness)}%)`;
  ...
  const saveAllImages = async () => {
    ...
    if (processed.length === 1) {
      ...
      setTimeout(() => downloadFile(processed[0].processedBlob, `brightness_${nameWithoutExt}${ext}`), 100);
      return;
    }
        ...
        <aside className="w-full lg:w-[320px] border-r border-slate-200 bg-slate-50 flex flex-col shrink-0 overflow-y-auto z-20">
          <div className="p-5 space-y-6">
            ...
            <div className={`flex flex-col gap-3 transition-opacity ${!activeFile ? 'opacity-50 pointer-events-none' : ''}`}>
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest pb-2 border-b border-slate-200">2. Brightness Adjustment</h3>

              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Brightness Level</span>
                  <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{formatBrightness(brightness)} ({getBrightnessLabel(brightness)})</span>
                </div>

                <input
                  type="range" min="-100" max="100" step="5"
                  value={brightness} onChange={(e) => setBrightness(parseInt(e.target.value, 10))}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PRESETS.map(preset => (
                    <button
                      ...
                      className={`py-1.5 px-2 text-[11px] font-bold rounded-lg border transition-all ${brightness === preset.value ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-[inset_0_2px_10px_rgba(59,130,246,0.05)]' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-700'}`}
                    >
                      {formatBrightness(preset.value)} {preset.label}
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className={`flex flex-col gap-2 mt-auto pt-4 ${!activeFile ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              >
                {isProcessing ? 'Processing...' : <><Sun className="w-4 h-4" /> Apply Brightness Changes</>}
              </button>
            </div>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-slate-100 relative border-l border-slate-200">
          {!activeFile ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8">
              ...
              <p className="text-sm mt-2 text-slate-400 text-center max-w-sm">Upload single or multiple images using the left panel to adjust brightness level.</p>
            </div>
          ) : (
            <>
              {/* Header */}
              <div className="flex items-center justify-between p-3 border-b border-slate-200 shrink-0 bg-slate-50 z-20">
                ...
                <div className="flex items-center gap-4">
                  <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full border ${activeFile.processedBlob ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                    {activeFile.processedBlob ? `✓ Brightness (${formatBrightness(activeFile.brightness)})` : 'Original Image'}
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 border-t border-slate-200 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 shadow-[0_-4px_25px_rgba(0,0,0,0.02)] z-20">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider hidden sm:block">
                  {activeFile.processedBlob ? `Brightness: ${formatBrightness(activeFile.brightness)} | Ready to save` : `Brightness: ${formatBrightness(brightness)} | Click Apply to process`}
                </span>
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <button
                    onClick={() => downloadFile(activeFile.processedBlob, `brightness_${activeFile.name.replace(/\.[^/.]+$/, "")}.${activeFile.file.type === "image/png" ? "png" : "jpg"}`)}
