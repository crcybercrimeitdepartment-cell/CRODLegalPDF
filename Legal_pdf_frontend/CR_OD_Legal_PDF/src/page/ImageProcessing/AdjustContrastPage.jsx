...
import {
  UploadCloud, X, Eclipse, Download, Package, ArrowLeft, Image as ImageIcon,
  Wand2, Settings2
} from 'lucide-react';
...
const AdjustContrast = ({ tool, onBack }) => {
  ...
  useEffect(() => {
    if (activeFile && (activeFile.contrast !== contrast || activeFile.mode !== mode)) {
      setUploadedFiles(prev => {
        const up = [...prev];
        if (up[activeIndex]) {
          up[activeIndex].contrast = contrast;
  ...
  }, [contrast, mode, activeFile, activeIndex]);

  // Sync state when active index changes
  useEffect(() => {
    if (activeFile) {
      setContrast(activeFile.contrast);
      setMode(activeFile.mode || 'manual');
    }
  }, [activeIndex]);

  // Helper for Displaying contrast values (+/-)
  ...
  const applyContrast = async () => {
    ...
    try {
      ...
      // Frontend Simulation using Canvas Filters
      if (mode === 'auto') {
        // Mock Auto Mode: slightly boost contrast & saturation dynamically
        ctx.filter = `contrast(135%) saturate(110%)`;
      } else {
        // Manual Mode
        const cssContrast = 100 + contrast;
        ctx.filter = `contrast(${Math.max(0, cssContrast)}%)`;
      }
      ...
      setIsProcessing(false);
      alert("Failed to adjust contrast.");
    }
  };
  ...
  const saveAllImages = async () => {
    ...
    if (processed.length === 1) {
      ...
      setTimeout(() => downloadFile(processed[0].processedBlob, `contrast_${nameWithoutExt}${ext}`), 100);
      return;
    }
        ...
        <aside className="w-full lg:w-[320px] border-r border-slate-200 bg-slate-50 flex flex-col shrink-0 overflow-y-auto z-20">
          <div className="p-5 space-y-6">
            ...
            <div className={`flex flex-col gap-3 transition-opacity ${!activeFile ? 'opacity-50 pointer-events-none' : ''}`}>
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest pb-2 border-b border-slate-200">2. Contrast Configuration</h3>

              {/* Mode Tabs */}
              <div className="grid grid-cols-2 gap-1 p-1 bg-slate-100 rounded-lg border border-slate-200 shadow-inner">
                <button
                  onClick={() => setMode('manual')}
                  className={`flex items-center justify-center gap-1.5 py-1.5 px-2 text-[11px] font-bold rounded-md transition-all ${mode === 'manual' ? 'bg-white text-blue-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/50'}`}
                >
                  <Settings2 className="w-3.5 h-3.5" /> Manual
                </button>
                <button
                  onClick={() => setMode('auto')}
                  className={`flex items-center justify-center gap-1.5 py-1.5 px-2 text-[11px] font-bold rounded-md transition-all ${mode === 'auto' ? 'bg-white text-blue-600 border border-slate-200 shadow-sm' : 'text-slate-500 hover:text-slate-800 hover:bg-slate-200/50'}`}
                >
                  <Wand2 className="w-3.5 h-3.5" /> Automatic
                </button>
              </div>
              ...
              {mode === 'manual' && (
                <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Contrast Intensity</span>
                    <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{formatContrast(contrast)} ({getContrastLabel(contrast)})</span>
                  </div>

                  <input
                    type="range" min="-100" max="100" step="5"
                    value={contrast} onChange={(e) => setContrast(parseInt(e.target.value, 10))}
                    className="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600 mb-4"
                  />
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {PRESETS.map(preset => (
                      <button
                        key={preset.value}
                        onClick={() => setContrast(preset.value)}
                        className={`py-1.5 px-2 text-[11px] font-bold rounded-lg border transition-all ${contrast === preset.value ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-[inset_0_2px_10px_rgba(59,130,246,0.05)]' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-700'}`}
                      >
                        {preset.label} ({formatContrast(preset.value)})
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {/* Auto Mode Box */}
              {mode === 'auto' && (
                <div className="bg-blue-50/50 p-4 rounded-xl border border-blue-100 shadow-sm flex gap-3 items-start">
                  <Wand2 className="w-6 h-6 text-blue-600 shrink-0 mt-0.5" />
                  <div>
                    <strong className="text-sm text-slate-800 block mb-1">Automatic Contrast Engine</strong>
                    <p className="text-xs text-slate-500 leading-relaxed font-medium">Automatically calculates dynamic cutoff limits and optimizes histogram tonal range for perfect contrast.</p>
                  </div>
                </div>
              )}
            ...
            <div className={`flex flex-col gap-2 mt-auto pt-4 ${!activeFile ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              >
                {isProcessing ? 'Processing...' : <><Eclipse className="w-4 h-4" /> Apply Contrast Adjustment</>}
              </button>
            </div>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-slate-100 relative border-l border-slate-200">
          {!activeFile ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8">
              ...
              <p className="text-sm mt-2 text-slate-400 text-center max-w-sm">Upload single or multiple images using the left panel to configure contrast settings.</p>
            </div>
          ) : (
            <>
              {/* Header */}
              <div className="flex items-center justify-between p-3 border-b border-slate-200 shrink-0 bg-slate-50 z-20">
                ...
                <div className="flex items-center gap-4">
                  <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full border ${activeFile.processedBlob ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                    {activeFile.processedBlob ? `✓ Contrast: ${activeFile.mode === 'auto' ? 'Auto' : formatContrast(activeFile.contrast)}` : 'Original Image'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex-1 flex items-center justify-center p-8 overflow-auto bg-slate-100 relative z-10">
                <div className="shadow-[0_8px_30px_rgba(0,0,0,0.06)] rounded-sm border border-slate-200 p-2 bg-white">
                  <img
                    src={activeFile.processedUrl || activeFile.dataUrl}
                    alt="Preview"
                    className="block max-w-full object-contain rounded-sm"
              ...
              <div className="p-4 border-t border-slate-200 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 shadow-[0_-4px_25px_rgba(0,0,0,0.02)] z-20">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider hidden sm:block">
                  {activeFile.processedBlob ? `Contrast applied | Ready to save` : `Click Apply to process`}
                </span>
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <button
                    onClick={() => downloadFile(activeFile.processedBlob, `contrast_${activeFile.name.replace(/\.[^/.]+$/, "")}.${activeFile.file.type === "image/png" ? "png" : "jpg"}`)}
                    disabled={!activeFile.processedBlob}
                    className="flex-1 sm:flex-none px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:bg-slate-200 disabled:text-slate-400 shadow-md"
                  >
                    <Download className="w-4 h-4" /> Save Adjusted Image
                  </button>
                </div>
              </div>
            </>
