...
import { 
  UploadCloud, X, Palette, Download, Package, ArrowLeft, Image as ImageIcon
} from 'lucide-react';

const PRESETS = [
  { label: 'Grayscale', value: -100 },
  { label: 'Muted', value: -50 },
  { label: 'Normal', value: 0 },
  { label: 'Vibrant', value: 50 },
  { label: 'Vivid Max', value: 100 }
];

const AdjustSaturation = ({ tool, onBack }) => {
  ...
  useEffect(() => {
    if (activeFile && activeFile.saturation !== saturation) {
      setUploadedFiles(prev => {
        const up = [...prev];
        if (up[activeIndex]) {
          up[activeIndex].saturation = saturation;
  ...
  }, [saturation, activeFile, activeIndex]);

  // Sync state when active index changes
  useEffect(() => {
    if (activeFile) {
      setSaturation(activeFile.saturation);
    }
  }, [activeIndex]);

  // Helper for Displaying saturation values (+/-)
  const formatSaturation = (val) => {
    if (val > 0) return `+${val}%`;
    if (val < 0) return `${val}%`;
    return '0%';
  };

  const getSaturationLabel = (val) => {
    if (val === -100) return 'Grayscale';
    if (val < 0) return 'Muted';
    if (val === 100) return 'Vivid Max';
    if (val > 0) return 'Vibrant';
    return 'Normal';
  };
  ...
  const applySaturation = async () => {
    ...
    try {
      ...
      // Frontend Simulation using Canvas Filters: saturate(100% is normal)
      const cssSaturation = 100 + saturation; 
      ctx.filter = `saturate(${Math.max(0, cssSaturation)}%)`;
      ...
      setIsProcessing(false);
      alert("Failed to adjust saturation.");
    }
  };
  ...
  const saveAllImages = async () => {
    ...
    if (processed.length === 1) {
      ...
      setTimeout(() => downloadFile(processed[0].processedBlob, `saturation_${nameWithoutExt}${ext}`), 100);
      return;
    }
        ...
        <aside className="w-full lg:w-[320px] border-r border-slate-200 bg-slate-50 flex flex-col shrink-0 overflow-y-auto z-20">
          <div className="p-5 space-y-6">
            ...
            <div className={`flex flex-col gap-3 transition-opacity ${!activeFile ? 'opacity-50 pointer-events-none' : ''}`}>
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest pb-2 border-b border-slate-200">2. Saturation Adjustment</h3>
              
              <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                <div className="flex justify-between items-center mb-3">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Color Saturation</span>
                  <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">{formatSaturation(saturation)} ({getSaturationLabel(saturation)})</span>
                </div>
                
                <input 
                  type="range" min="-100" max="100" step="5" 
                  value={saturation} onChange={(e) => setSaturation(parseInt(e.target.value, 10))}
                ...
                <div className="grid grid-cols-2 gap-2">
                  {PRESETS.map((preset, i) => (
                    <button 
                      key={preset.value}
                      onClick={() => setSaturation(preset.value)}
                      className={`py-1.5 px-2 text-[11px] font-bold rounded-lg border transition-all ${saturation === preset.value ? 'bg-blue-50 border-blue-500 text-blue-600 shadow-[inset_0_2px_10px_rgba(59,130,246,0.05)]' : 'bg-white border-slate-200 text-slate-500 hover:border-slate-300 hover:text-slate-700'} ${i === PRESETS.length - 1 ? 'col-span-2' : ''}`}
                    >
                      {preset.label} ({formatSaturation(preset.value)})
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className={`flex flex-col gap-2 mt-auto pt-4 ${!activeFile ? 'opacity-50 pointer-events-none' : ''}`}>
              ...
              >
                {isProcessing ? 'Processing...' : <><Palette className="w-4 h-4" /> Apply Saturation Changes</>}
              </button>
            </div>
        ...
        <main className="flex-1 flex flex-col min-w-0 bg-slate-100 relative border-l border-slate-200">
          {!activeFile ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400 p-8">
              ...
              <p className="text-sm mt-2 text-slate-400 text-center max-w-sm">Upload single or multiple images using the left panel to adjust color saturation.</p>
            </div>
          ) : (
            <>
              {/* Header */}
              <div className="flex items-center justify-between p-3 border-b border-slate-200 shrink-0 bg-slate-50 z-20">
                ...
                <div className="flex items-center gap-4">
                  <span className={`text-[10px] font-bold uppercase px-2 py-1 rounded-full border ${activeFile.processedBlob ? 'bg-emerald-50 text-emerald-600 border-emerald-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                    {activeFile.processedBlob ? `✓ Saturation (${formatSaturation(activeFile.saturation)})` : 'Original Image'}
                  </span>
                </div>
              </div>
              ...
              <div className="p-4 border-t border-slate-200 shrink-0 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50 shadow-[0_-4px_25px_rgba(0,0,0,0.02)] z-20">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider hidden sm:block">
                  {activeFile.processedBlob ? `Saturation: ${formatSaturation(activeFile.saturation)} | Ready to save` : `Saturation: ${formatSaturation(saturation)} | Click Apply to process`}
                </span>
                <div className="flex gap-2 w-full sm:w-auto">
                  ...
                  <button 
                    onClick={() => downloadFile(activeFile.processedBlob, `saturation_${activeFile.name.replace(/\.[^/.]+$/, "")}.${activeFile.file.type === "image/png" ? "png" : "jpg"}`)}
