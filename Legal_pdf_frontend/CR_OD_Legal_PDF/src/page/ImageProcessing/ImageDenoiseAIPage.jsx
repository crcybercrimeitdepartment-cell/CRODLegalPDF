...
import { 
  ...
  CheckCircle2, Download, FileArchive, RefreshCw, Wand2, 
  ZoomIn, ZoomOut, Maximize, Feather, StarHalf, Star
} from 'lucide-react';

const ImageDenoiseAI = ({ tool, onBack }) => {
  // State
  ...
  const addFiles = (files) => {
    ...
    const newItems = validFiles.map(file => {
      const id = 'denoise_' + Math.random().toString(36).substr(2, 9);
      return {
        id,
        file,
  ...
  // --- 2. APPLY DENOISE (Simulated) ---
  const applyDenoise = async () => {
    if (!activeItem) return;
    lockUI("Removing digital noise and grain...");
        ...
        <main className="flex-1 flex flex-col min-w-0 relative border-l border-slate-200 bg-slate-50 p-4 sm:p-5">
          <div className="flex-1 flex flex-col h-full gap-4">
            ...
            <div className={`grid grid-cols-3 gap-3 bg-slate-100 p-4 rounded-xl border border-slate-200 shrink-0 shadow-lg transition-opacity ${!activeItem ? 'opacity-50 pointer-events-none' : ''}`}>
              {[
                { id: 'low', title: 'Low', desc: 'Light grain', icon: <Feather className="w-5 h-5 mb-1" /> },
                { id: 'medium', title: 'Medium', desc: 'Recommended', icon: <StarHalf className="w-5 h-5 mb-1" /> },
                { id: 'high', title: 'High', desc: 'Heavy noise', icon: <Star className="w-5 h-5 mb-1" /> }
              ].map(lvl => (
                <div 
                  key={lvl.id}
                  onClick={() => {
            ...
            >
              ...
              {!activeItem ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center overflow-visible">
                  ...
                  >
                    {/* Mock visual effect: Simulate noisy vs smooth. We use CSS contrast/sepia to simulate the change */}
                    <img 
                      ...
                      style={{ 
                        filter: (activeItem.status === 'completed' && !activeItem.showOriginal) 
                          ? (activeItem.level === 'high' ? 'blur(0.5px) contrast(1.1)' : activeItem.level === 'low' ? 'blur(0px) contrast(1.05)' : 'blur(0.2px) contrast(1.08)') 
                          : 'none' // original
                      }}
                    />
                  </div>
              ...
              <div className="px-5 py-4 bg-slate-100 border border-slate-200 rounded-xl flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0 shadow-lg">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-slate-500 font-medium hidden sm:block">
                    {activeItem.status === 'waiting' && "Ready to apply denoise."}
                    {activeItem.status === 'processing' && "AI is analyzing and removing noise..."}
                    {activeItem.status === 'completed' && "Image denoised successfully."}
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-end">
                  ...
                  {activeItem.status === 'completed' && (
                    <div className="flex items-center gap-2 cursor-pointer bg-white px-3 py-1.5 rounded-full border border-slate-200" onClick={toggleView}>
                      ...
                      <span className={`text-[10px] font-semibold ${!activeItem.showOriginal ? 'text-emerald-400' : 'text-slate-400'}`}>Denoised</span>
                    </div>
                  )}

                  {activeItem.status === 'completed' ? (
                    <button 
                      onClick={() => downloadFile(activeItem.localUrl, `denoise_${activeItem.file.name}`)}
                    ...
                    >
                      <CheckCircle2 className="w-3.5 h-3.5" /> Apply Denoise
                    </button>
                  )}
...
const ActiveStatusBadge = ({ status }) => {
  ...
  const labels = {
    waiting: 'READY', processing: 'PROCESSING...', completed: 'DENOISED', failed: 'FAILED'
  };
