...
export default function DownloadOptimizedPDFPage() {
      ...
      <div className="w-full max-w-6xl relative z-10">
        <div className="text-center max-w-2xl mx-auto mt-8 mb-8 px-4">
          ...
          <p className="text-xs sm:text-sm text-slate-600 font-medium leading-relaxed">
             Reduce file size and optimize resources for fast web delivery and sharing.
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 justify-center items-start w-full">
            ...
            <div className="bg-white/70 backdrop-blur-xl border border-white shadow-2xl rounded-3xl p-6 sm:p-8 relative overflow-hidden flex-1">
              ...
              >
                ...
                <p className="drop-text text-xl font-bold text-slate-800 mb-2 transition-colors group-hover:text-indigo-900">
                  {pdfFiles.length > 0 ? 'Add More PDFs' : 'Drag & drop PDFs here'}
                </p>
                  ...
                  <div className="flex w-full h-full gap-4 overflow-x-auto pb-2 custom-scrollbar snap-x">
                      {pdfFiles.map((file, idx) => (
                          <div key={idx} className="flex-shrink-0 w-full h-full relative snap-center bg-white shadow-sm border border-slate-200 rounded-xl overflow-hidden">
                              <div className="absolute top-0 left-0 w-full bg-slate-800/80 text-white text-xs px-3 py-1 font-medium z-10 truncate backdrop-blur-sm">
                                  {file.name}
                              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="p-5 bg-indigo-50/50 rounded-2xl shadow-sm border border-indigo-100/50">
                   ...
                   <p className="text-sm text-slate-600">
                     Configure how your PDF should be compressed and optimized.
                   </p>
                </div>

                <div className="space-y-3">
                  <label className="flex items-start gap-3 p-3 bg-white border border-slate-200 rounded-xl cursor-pointer hover:bg-indigo-50/50 hover:border-indigo-200 transition-colors">
                    <input 
                      type="checkbox" 
                      className="mt-1 w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
                      checked={settings.compressStreams}
                      onChange={(e) => setSettings({...settings, compressStreams: e.target.checked})}
                    />
                    <div>
                      <strong className="block text-sm font-semibold text-slate-700">Compress Object Streams</strong>
                      <span className="block text-xs text-slate-500 mt-0.5">Re-organize structural content streams into compressed objects.</span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 p-3 bg-white border border-slate-200 rounded-xl cursor-pointer hover:bg-indigo-50/50 hover:border-indigo-200 transition-colors">
                    <input 
                      type="checkbox" 
                      className="mt-1 w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
                      checked={settings.optimizeResources}
                      onChange={(e) => setSettings({...settings, optimizeResources: e.target.checked})}
                    />
                    <div>
                      <strong className="block text-sm font-semibold text-slate-700">Optimize Resources</strong>
                      <span className="block text-xs text-slate-500 mt-0.5">Clean up internal cross-references and streamline PDF page indexes.</span>
                    </div>
                  </label>
                  ...
                  <label className="flex items-start gap-3 p-3 bg-white border border-slate-200 rounded-xl cursor-pointer hover:bg-indigo-50/50 hover:border-indigo-200 transition-colors">
                    ...
                    />
                    <div>
                      <strong className="block text-sm font-semibold text-slate-700">Remove Duplicate Objects</strong>
                      <span className="block text-xs text-slate-500 mt-0.5">Merge identical content streams, images, and font descriptors.</span>
                    </div>
                  </label>

                  <label className="flex items-start gap-3 p-3 bg-white border border-slate-200 rounded-xl cursor-pointer hover:bg-amber-50/50 hover:border-amber-200 transition-colors border-l-4 border-l-amber-400">
                    <input 
                      type="checkbox" 
                      className="mt-1 w-4 h-4 text-indigo-600 rounded border-slate-300 focus:ring-indigo-500"
                    ...
                    />
                    <div>
                      <strong className="block text-sm font-semibold text-slate-700">Preserve Quality</strong>
                      <span className="block text-xs text-slate-500 mt-0.5">Avoid aggressive image scaling to keep original resolution ratios.</span>
                    </div>
                  </label>
                </div>
          ...
          {isProcessing && (
            <div className="w-full lg:max-w-md bg-white/70 backdrop-blur-xl border border-white shadow-2xl rounded-3xl p-10 flex flex-col items-center justify-center relative overflow-hidden animate-fade-in-up min-h-[400px]">
              ...
              <p className="text-slate-500 text-center text-sm">Please wait while we optimize your document.</p>
            </div>
          )}
