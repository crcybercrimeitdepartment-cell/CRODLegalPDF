...
export default function HTMLtoPDFPage({ onBack }) {
  ...
  const toolDesc = "Convert HTML to PDF — upload a file or paste code with live preview";
  const apiSlug = "html-to-pdf";
  ...
  const handleProcessFromCode = async () => {
    if (!htmlCode.trim()) { setError('Please enter some HTML code first.'); return; }
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/convert-to-pdf/${apiSlug}/render`, {
        ...
        body: JSON.stringify({ code: htmlCode, filename: 'output.pdf' }),
      });
      if (!res.ok) {
    ...
    <div className="flex-1 flex flex-col w-full bg-transparent relative z-20 min-h-screen">
      ...
      {!isDone && (
        <div className="flex justify-center gap-2 mb-6 px-4">
          <button
            ...
            className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all cursor-pointer ${mode === 'code' ? 'bg-[#1e2a52] text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          >
            <Code size={16} /> Paste Code
          </button>
          <button
            onClick={() => setMode('file')}
            className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all cursor-pointer ${mode === 'file' ? 'bg-[#1e2a52] text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
          >
      ...
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 pb-14">
        <div className="bg-white rounded-3xl border border-slate-200/80 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 sm:p-8">
          ...
          {!isDone && mode === 'code' && (
            <div>
              <p className="text-sm font-bold text-slate-700 mb-3">Write or paste your HTML code — see live preview on the right:</p>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4" style={{ minHeight: '420px' }}>
                <CodeEditor language="html" value={htmlCode} onChange={setHtmlCode} height="420px" />
                <LivePreview html={htmlCode} height="420px" />
              </div>
              <div className="mt-6 text-center">
                {isProcessing ? (
                  <div className="flex flex-col items-center justify-center p-8 bg-[#f8faf7] border border-slate-200/80 rounded-2xl min-h-[100px]">
                    ...
                    <p className="text-xs sm:text-sm font-bold text-[#1e2a52] animate-pulse">Converting HTML to PDF...</p>
                  </div>
                ) : (
                  <button onClick={handleProcessFromCode} disabled={!htmlCode.trim()} className="bg-[#1e2a52] hover:bg-[#16203e] text-white px-10 py-3.5 rounded-full font-bold shadow-lg transition-all text-sm cursor-pointer inline-flex items-center gap-2 hover:scale-105 active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:scale-100">
                    Convert to PDF
                  </button>
                )}
              </div>
          ...
          {!isDone && mode === 'file' && (
            <div>
              ...
              >
                ...
                <p className="text-base sm:text-lg font-bold text-[#1e2a52] mb-1">{file ? file.name : 'Drop HTML file here or click to browse'}</p>
                <p className="text-xs sm:text-sm text-slate-500">Accepted: <span className="font-semibold text-[#1e2a52]">.html, .htm files</span></p>
              </div>
                ...
                <div className="mt-6">
                  <div className="flex flex-col items-center gap-3 bg-slate-50 border border-slate-200/80 rounded-xl p-4">
                    ...
                    <div className="flex-1 min-w-0 text-center w-full flex items-center justify-between">
                      ...
                      <button onClick={() => { setFile(null); }} className="p-2 bg-slate-200 hover:bg-red-100 text-slate-500 hover:text-red-600 rounded-full transition-colors cursor-pointer shrink-0">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  </div>
                  <div className="mt-6 text-center">
                    {isProcessing ? (
                      ...
                      <button onClick={handleProcessFromFile} className="bg-[#1e2a52] hover:bg-[#16203e] text-white px-10 py-3.5 rounded-full font-bold shadow-lg transition-all text-sm cursor-pointer inline-flex items-center gap-2 hover:scale-105 active:scale-95">
                        Convert to PDF
                      </button>
                    )}
                  </div>
              ...
              {pdfPreviewUrl && (
                <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-sm">
                  <div className="flex items-center justify-between bg-slate-100 px-4 py-2">
                    ...
                    <button onClick={() => setShowPdfPreview(v => !v)} className="text-xs text-[#1e2a52] font-bold hover:underline cursor-pointer">
                      {showPdfPreview ? 'Hide' : 'Show'} Preview
                    </button>
                  </div>
                  {showPdfPreview && (
                    <iframe src={pdfPreviewUrl} title="PDF Preview" className="w-full" style={{ height: '500px', border: 'none' }} />
                  )}
                </div>
              )}
              <div className="flex justify-center gap-3 flex-wrap">
                {pdfPreviewUrl && !showPdfPreview && (
                  <button onClick={() => setShowPdfPreview(true)} className="inline-flex items-center gap-2 bg-[#1e2a52] hover:bg-[#16203e] text-white px-6 py-3 rounded-full font-bold shadow-md transition-all text-sm cursor-pointer hover:scale-105">
