...
import { Upload, FileText, Download, CheckCircle2, ArrowLeft, X, AlertCircle, Eye, Plus } from 'lucide-react';

const API_BASE_URL = import.meta.env.VITE_API_URL || import.meta.env.VITE_BACKEND_URL || '';

export default function MultipleFilestoPDFPage({ onBack }) {
  const toolName = "Multiple Files to PDF";
  const toolDesc = "Combine multiple documents (Word, Images, Excel, etc.) into a single PDF";
  const apiSlug = "multiple_files-to-pdf";
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      processForm.append('filename', uploadData.filenames?.[0] || 'merged');
  ...
  const getFileIcon = (f) => {
    ...
    const colors = { pdf: 'text-red-500', doc: 'text-blue-600', docx: 'text-blue-600', xls: 'text-green-600', xlsx: 'text-green-600', ppt: 'text-orange-500', pptx: 'text-orange-500', jpg: 'text-purple-500', jpeg: 'text-purple-500', png: 'text-purple-500', txt: 'text-slate-500' };
          ...
          {!isDone && (
            <>
              ...
              >
                ...
                <p className="text-xs sm:text-sm text-slate-500">Select multiple files — Word, Images, Excel, Text, etc.</p>
                <p className="text-xs text-slate-400 mt-1">All files will be combined into a single PDF</p>
              </div>

              {files.length > 0 && (
                <div className="mt-5">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-sm font-bold text-slate-700">{files.length} file{files.length !== 1 ? 's' : ''} selected</span>
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {files.map((f, idx) => (
                      <div key={idx} className="flex items-center gap-3 bg-slate-50 border border-slate-200/80 rounded-xl px-4 py-2.5">
                        <FileText className={`w-5 h-5 shrink-0 ${getFileIcon(f)}`} />
                  ...
                  <div className="mt-6 text-center">
                    {isProcessing ? (
                      <div className="flex flex-col items-center justify-center p-8 bg-[#f8faf7] border border-slate-200/80 rounded-2xl min-h-[120px]">
                        ...
                        <p className="text-xs sm:text-sm font-bold text-[#1e2a52] animate-pulse">Merging files into PDF... Please wait!</p>
                      </div>
                    ) : (
          ...
          {isDone && (
            <div className="space-y-5">
              <div className="flex items-center justify-center gap-2 text-emerald-600 font-bold text-base pt-2">
                <CheckCircle2 className="w-6 h-6" /> Conversion Complete! {files.length} files merged.
              </div>
              {pdfPreviewUrl && (
