...
import {
  Cloud, MousePointer2, Undo2, Redo2, Trash2, ZoomIn, ZoomOut, Maximize, Upload, Download, ArrowLeft, MessageSquare
} from 'lucide-react';
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl p-3 flex flex-col md:flex-row items-center gap-4 border border-gray-200 z-50 transition-all duration-300 w-max max-w-[98%] overflow-x-auto">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button onClick={() => setTool('cloud')} className={`p-2 rounded-xl transition-all ${tool === 'cloud' ? 'bg-blue-100 text-blue-600 shadow-sm' : 'hover:bg-gray-100 text-gray-600'}`} title="Cloud Tool"><Cloud size={20} /></button>
      ...
      <div className="flex flex-col gap-2 w-32 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          }} className="w-full cursor-pointer accent-blue-500" title="Cloud Scallop Radius" />
        </div>
      </div>
...
function DrawingBoard({
  ...
  opacity, cloudRadius, pageNumber
}) {
  ...
  const handlePointerDown = (e) => {
    if (e.target.closest('.resize-handle') || (tool === 'select' && e.target.tagName !== 'svg' && e.target.tagName !== 'path')) {
      return;
    }
  ...
  useEffect(() => {
    ...
    pageNumber, strokeColor, fillColor, borderWidth, opacity, cloudRadius, dragStartVertices
  ]);

  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 z-20 ${tool === 'cloud' ? 'cursor-crosshair' : 'cursor-default'}`}
    ...
    >
      ...
      >
        ...
        {isDrawing && currentVertices.length > 0 && (
          <path
            d={generateCloudPath(currentVertices, currentMousePos, cloudRadius)}
...
export function CloudToolRenderer({ ann, zoom, isSelected, tool, handleAnnotationPointerDown, handleResizePointerDown, pixelsPerInch }) {
...
export default function CloudAnnotationPage({ onBack }) {
  ...
  const [tool, setTool] = useState('cloud');
