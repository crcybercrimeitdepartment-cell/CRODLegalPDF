...
import {
  Highlighter, MousePointer2, Undo2, Redo2, Trash2, ZoomIn, ZoomOut, Maximize, Upload, Download, ArrowLeft, MessageSquare
} from 'lucide-react';
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl p-3 flex flex-col md:flex-row items-center gap-4 border border-gray-200 z-50 transition-all duration-300 w-max max-w-[98%] overflow-x-auto">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button onClick={() => setTool('highlight')} className={`p-2 rounded-xl transition-all ${tool === 'highlight' ? 'bg-blue-100 text-blue-600 shadow-sm' : 'hover:bg-gray-100 text-gray-600'}`} title="Highlight Tool"><Highlighter size={20} /></button>
...
function DrawingBoard({
  ...
  setSelectedAnnotationId, zoom, pan, tool, strokeColor,
  opacity, pageNumber
}) {
  ...
  }, [tool, zoom, pan, strokeColor, opacity, addAnnotation, pageNumber]);
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 overflow-hidden z-20 ${tool === 'highlight' ? 'pointer-events-none' : 'pointer-events-auto'}`}
...
export function HighlightToolRenderer({ ann, zoom, isSelected, tool, handleAnnotationPointerDown, handleResizePointerDown, pixelsPerInch }) {
...
export default function HighlightTextPage({ onBack }) {
  ...
  const [strokeColor, setStrokeColor] = useState('#fef08a'); // Default yellow
...
return (
    <div className="relative w-screen h-screen bg-gray-200 flex flex-col">
      ...
      />
      <div className={`flex-1 overflow-auto p-4 pt-24 pb-20 flex justify-center selection:bg-blue-200 selection:bg-opacity-40 ${tool === 'highlight' ? 'cursor-text' : ''}`}>
              ...
              {Array.from(new Array(numPages), (el, index) => (
                <div key={`page_${index + 1}`} className="relative bg-white shadow-xl">
                  ...
                  <DrawingBoard
                    ...
                    tool={tool} strokeColor={strokeColor} opacity={opacity}
                  />
                </div>
              ))}
            </Document>
