...
import {
  Activity, MousePointer2, Undo2, Redo2, Trash2, ZoomIn, ZoomOut, Maximize, Upload, Download, ArrowLeft, MessageSquare
} from 'lucide-react';
...
function Toolbar({
  ...
  lineStyle, setLineStyle, joinStyle, setJoinStyle,
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl p-3 flex flex-col md:flex-row items-center gap-4 border border-gray-200 z-50 transition-all duration-300 w-max max-w-[98%] overflow-x-auto">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button onClick={() => setTool('polyline')} className={`p-2 rounded-xl transition-all ${tool === 'polyline' ? 'bg-blue-100 text-blue-600 shadow-sm' : 'hover:bg-gray-100 text-gray-600'}`} title="Polyline Tool"><Activity size={20} /></button>
      ...
      {/* LINE STYLES AND JOINS */}
      <div className="flex flex-col gap-2 shrink-0">
        <div className="flex gap-2">
            <select value={lineStyle} onChange={(e) => {
              setLineStyle(e.target.value);
              if (selectedAnnotationId) updateAnnotation(selectedAnnotationId, { lineStyle: e.target.value });
...
function DrawingBoard({
  ...
  opacity, lineStyle, joinStyle, pageNumber
}) {
  ...
  const handlePointerDown = (e) => {
    if (e.target.closest('.resize-handle') || (tool === 'select' && e.target.tagName !== 'svg' && e.target.tagName !== 'polyline')) {
      return;
    }
  ...
  useEffect(() => {
    ...
    pageNumber, strokeColor, borderWidth, opacity, lineStyle, joinStyle, dragStartVertices
  ]);
  ...
  const renderPolyline = (vertices, previewCoord) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 z-20 ${tool === 'polyline' ? 'cursor-crosshair' : 'cursor-default'}`}
      ...
      >
        ...
        {isDrawing && currentVertices.length > 0 && (
          <polyline
            ...
            strokeDasharray={getStrokeDashArray(lineStyle, borderWidth)}
          />
        )}
      </svg>
    </div>
...
export function PolylineToolRenderer({ ann, zoom, isSelected, tool, handleAnnotationPointerDown, handleResizePointerDown, pixelsPerInch }) {
          ...
          return (
            <g key={ann.id}>
              ...
              <polyline
                ...
                opacity={ann.opacity}
                strokeLinejoin={ann.joinStyle}
                strokeDasharray={getStrokeDashArray(ann.lineStyle, ann.borderWidth)}
                className={`pointer-events-auto ${tool === 'select' ? 'cursor-move' : ''}`}
                onPointerDown={(e) => handleAnnotationPointerDown(e, ann)}
              />
...
export default function PolylineToolPage({ onBack }) {
              ...
              {Array.from(new Array(numPages), (el, index) => (
                <div key={`page_${index + 1}`} className="relative bg-white shadow-xl">
                  ...
                  <DrawingBoard
                    ...
                    lineStyle={lineStyle} joinStyle={joinStyle}
                  />
                </div>
              ))}
            </Document>
