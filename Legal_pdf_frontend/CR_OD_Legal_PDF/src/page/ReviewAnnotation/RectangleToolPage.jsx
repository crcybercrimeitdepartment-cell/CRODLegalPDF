...
import {
  Square, MousePointer2, Undo2, Redo2, Trash2, ZoomIn, ZoomOut, Maximize, Upload, Download, ArrowLeft, MessageSquare
} from 'lucide-react';
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl p-3 flex flex-col md:flex-row items-center gap-4 border border-gray-200 z-50 transition-all duration-300 w-max max-w-[98%] overflow-x-auto">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button onClick={() => setTool('rectangle')} className={`p-2 rounded-xl transition-all ${tool === 'rectangle' ? 'bg-blue-100 text-blue-600 shadow-sm' : 'hover:bg-gray-100 text-gray-600'}`} title="Rectangle Tool"><Square size={20} /></button>
...
function DrawingBoard({
  ...
  const handlePointerDown = (e) => {
    // If we click a resize handle or existing annotation, let their handlers take it
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 z-20 ${tool === 'rectangle' ? 'cursor-crosshair' : 'cursor-default'}`}
...
export function RectangleToolRenderer({ ann, zoom, isSelected, tool, handleAnnotationPointerDown, handleResizePointerDown, pixelsPerInch }) {
          ...
          return (
            <g key={ann.id}>
              ...
              />
              {/* Invisible larger hit area for easier selecting of unfilled rectangles */}
...
export default function RectangleToolPage({ onBack }) {
