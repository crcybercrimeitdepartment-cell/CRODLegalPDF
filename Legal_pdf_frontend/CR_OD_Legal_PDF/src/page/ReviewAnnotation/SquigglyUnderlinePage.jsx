...
import {
  Activity as SquigglyIcon, MousePointer2, Undo2, Redo2, Trash2, ZoomIn, ZoomOut, Maximize, Upload, Download, ArrowLeft, MessageSquare
} from 'lucide-react';
  ...
  return (
    <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/80 backdrop-blur-xl shadow-2xl rounded-2xl p-3 flex flex-col md:flex-row items-center gap-4 border border-gray-200 z-50 transition-all duration-300 w-max max-w-[98%] overflow-x-auto">
      ...
      <div className="flex items-center gap-2 shrink-0">
        <button onClick={() => setTool('squiggly')} className={`p-2 rounded-xl transition-all ${tool === 'squiggly' ? 'bg-blue-100 text-blue-600 shadow-sm' : 'hover:bg-gray-100 text-gray-600'}`} title="Squiggly Tool"><SquigglyIcon size={20} /></button>
      ...
      <div className="flex items-center gap-2 shrink-0 w-max">
        ...
        >
          ...
          <option value="small">Small Wave</option>
          <option value="large">Large Wave</option>
...
function DrawingBoard({
  ...
  setSelectedAnnotationId, zoom, pan, tool, strokeColor, waveStyle, strokeWidth,
  opacity, pageNumber
}) {
  const containerRef = useRef(null);

  // Handle Text Selection for Squiggly Underline
  useEffect(() => {
    const handleMouseUp = () => {
      if (tool !== 'squiggly') return;
  ...
  }, [tool, zoom, pan, strokeColor, waveStyle, strokeWidth, opacity, addAnnotation, pageNumber]);
  ...
  return (
    <div
      ref={containerRef}
      className={`absolute inset-0 overflow-hidden z-20 ${tool === 'squiggly' ? 'pointer-events-none' : 'pointer-events-auto'}`}
    ...
    >
      <svg
        ...
        style={{
          ...
          transformOrigin: '0 0',
          pointerEvents: tool === 'squiggly' ? 'none' : 'auto'
        }}
      >
        <AnnotationLayer 
...
export function SquigglyToolRenderer({ ann, zoom, isSelected, tool, handleAnnotationPointerDown, handleResizePointerDown, pixelsPerInch }) {
          ...
          if (ann.type === 'squiggly') {
            return (
              ...
              >
                {ann.rects.map((r, i) => {
                  const lineY = r.y + r.h + 3; // 3px visual padding below text for waves
                ...
                {ann.rects.map((r, i) => (
                  <rect key={`hit_${i}`} x={r.x} y={r.y} width={r.w} height={r.h + 10} fill="transparent" />
                ))}
                ...
                {isSelected && ann.rects.length > 0 && (
                  <g>
                    {ann.rects.map((r, i) => (
                      <rect 
                        key={`sel_${i}`}
                        x={r.x - 2} y={r.y - 2} width={r.w + 4} height={r.h + 10 + ann.width * 2} 
...
export default function SquigglyUnderlinePage({ onBack }) {
  ...
  const [tool, setTool] = useState('squiggly');
...
return (
    <div className="relative w-screen h-screen bg-gray-200 flex flex-col">
      ...
      />
      <div className={`flex-1 overflow-auto p-4 pt-24 pb-20 flex justify-center selection:bg-blue-200 selection:bg-opacity-40 ${tool === 'squiggly' ? 'cursor-text' : ''}`}>
              ...
              {Array.from(new Array(numPages), (el, index) => (
                <div key={`page_${index + 1}`} className="relative bg-white shadow-xl">
                  ...
                  <DrawingBoard
                    ...
                    tool={tool} strokeColor={strokeColor} strokeWidth={strokeWidth} waveStyle={waveStyle} opacity={opacity}
                  />
                </div>
              ))}
            </Document>
