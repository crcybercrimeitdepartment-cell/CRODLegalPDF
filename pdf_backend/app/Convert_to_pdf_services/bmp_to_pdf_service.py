...
class BmpToPdfService:
    ...
    async def process(
        ...
        logger.info(f"BMP→PDF: {len(filenames)} image(s) → '{safe_name}' [{output_path.stat().st_size} bytes]")
...
bmp_to_pdf_service = BmpToPdfService()
