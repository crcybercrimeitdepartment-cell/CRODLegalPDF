...
def draw_circle_on_page(page: fitz.Page, ann: Dict[str, Any]):
    """
    Burn a vector circle / ellipse shape annotation into a PyMuPDF page.
    """
    pts = ann.get("points", [])
    if len(pts) < 2:
        return
...
def apply_circle_annotations_to_pdf(input_pdf_path: Path, output_pdf_path: Path, annotations: List[Dict[str, Any]]) -> bool:
    """
    Burn all circle / ellipse shape annotations into the PDF using PyMuPDF (fitz).
    """
    ...
    for page_num in range(1, len(doc) + 1):
        ...
        for ann in annots:
            ...
            if ann_type in ["circle", "ellipse", "shape_ellipse", "oval"]:
