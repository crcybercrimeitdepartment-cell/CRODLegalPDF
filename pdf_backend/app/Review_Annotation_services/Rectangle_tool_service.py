...
def draw_rectangle_on_page(page: fitz.Page, ann: Dict[str, Any]):
    """
    Burn a vector rectangle shape annotation into a PyMuPDF page.
    """
    pts = ann.get("points", [])
    if len(pts) < 2:
        return
    ...
    stroke_color_hex = ann.get("color", ann.get("borderColor", "#2563EB"))
    ...
    rect_style = str(ann.get("rectStyle", "solid")).lower()

    shape = page.new_shape()
    shape.draw_rect(rect)

    dashes = "(3, 3)" if rect_style == "dashed" else None
...
def apply_rectangle_annotations_to_pdf(input_pdf_path: Path, output_pdf_path: Path, annotations: List[Dict[str, Any]]) -> bool:
    """
    Burn all rectangle shape annotations into the PDF using PyMuPDF (fitz).
    """
    ...
    for page_num in range(1, len(doc) + 1):
        ...
        for ann in annots:
            ...
            if ann_type in ["rectangle", "rect", "shape_rectangle", "box"]:
