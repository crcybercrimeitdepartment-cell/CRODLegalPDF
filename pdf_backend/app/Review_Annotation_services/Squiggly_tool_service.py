...
def generate_squiggly_points(p1: fitz.Point, p2: fitz.Point, amplitude: float = 2.5, wavelength: float = 6.0) -> List[fitz.Point]:
...
def draw_squiggly_on_page(page: fitz.Page, ann: Dict[str, Any]):
    """
    Burn a vector squiggly wavy underline annotation into a PyMuPDF page.
    """
    pts = ann.get("points", [])
    ...
    amplitude = float(ann.get("waveAmplitude", 2.5))
    wavelength = float(ann.get("waveLength", 6.0))

    p1 = fitz.Point(float(pts[0].get("x", 0)), float(pts[0].get("y", 0)))
    p2 = fitz.Point(float(pts[1].get("x", 0)), float(pts[1].get("y", 0)))

    shape = page.new_shape()
    wavy_points = generate_squiggly_points(p1, p2, amplitude=amplitude, wavelength=wavelength)
...
def apply_squiggly_annotations_to_pdf(input_pdf_path: Path, output_pdf_path: Path, annotations: List[Dict[str, Any]]) -> bool:
    """
    Burn all squiggly underline annotations into the PDF using PyMuPDF (fitz).
    """
    ...
    for page_num in range(1, len(doc) + 1):
        ...
        for ann in annots:
            ...
            if ann_type in ["squiggly", "wavy_underline", "text_squiggly"]:
